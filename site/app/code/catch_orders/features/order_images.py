...
def get(order_info: dict, sku: str, item_id: str) -> list[tuple[str, str]]:
    """
    ...
    images_item = next(
        (
            item 
            for item in order_info['items'][1::2] 
            if (
    ...
    for index, (base_url, print_url) in enumerate(
        ...
        with download_session.get(print_url, stream=True) as download_request:
            with open(f'GotImages/print{index*2}.{print_url.split(".")[-1]}', 'wb') as image:
        ...
        with download_session.get(base_url, stream=True) as download_request:
            with open(f'GotImages/base{index*2+1}.{base_url.split(".")[-1]}', 'wb') as image:
                for chunk in download_request.iter_content(chunk_size=8192):
                    image.write(chunk)
        images.append((f'GotImages/print{index*2}.{print_url.split(".")[-1]}', f'GotImages/base{index*2+1}.{base_url.split(".")[-1]}'))
