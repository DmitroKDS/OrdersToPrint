...
def index():
    ...
    if request.method == 'POST':
        ...
        use_old_items = request.form.get('use_old_items') == "on"
        import logging
        logging.info(use_old_items)

        proccess_data = [date, orders, use_old_items]
...
def proccess():
    ...
    orders=catch_orders(proccess_data[0], proccess_data[1], proccess_data[2])
...
def get_log():
    ...
    if isinstance(proccess_data, list):
        return {"log":orders.catch_orders_data[0], "percentage":int(orders.catch_orders_data[1])}
    else:
